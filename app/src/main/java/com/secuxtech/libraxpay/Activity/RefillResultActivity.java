...
public class RefillResultActivity extends BaseActivity  {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        textViewTitle.setText("Refill Result");

        Boolean result = getIntent().getExtras().getBoolean(RefillActivity.REFILL_RESULT);
        mResult = result;

        ImageView imgviewRet = findViewById(R.id.imageView_result);
        int color = ContextCompat.getColor(this, R.color.colorPaymentFail);
        String resultStr = "Refill Failed";
        if (result){
            resultStr = "Refill Successful";
            ...
            String error = getIntent().getStringExtra(RefillActivity.REFILL_ERROR);
        ...
        String amountStr = getIntent().getStringExtra(RefillActivity.REFILL_AMOUNT);
        ...
        String date = getIntent().getStringExtra(RefillActivity.REFILL_DATE);
        TextView textviewDate = findViewById(R.id.textView_payment_date);
        textviewDate.setText(date);

        String storename = getIntent().getStringExtra(RefillActivity.REFILL_STORENAME);
